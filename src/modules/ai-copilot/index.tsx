...
export default function Copilot(){
 ...
 <div className="intake-buttons"><label className="secondary"><Camera size={19}/>Camera<input className="visually-hidden" aria-label="Chụp ảnh để AI trích xuất" type="file" accept="image/*" capture="environment" disabled={importing||waiting} onChange={e=>{chooseImage(e.target.files?.[0]);e.target.value=''}}/></label><label className="secondary"><ImagePlus size={19}/>Chọn ảnh<input className="visually-hidden" aria-label="Chọn ảnh hoặc screenshot để AI trích xuất" type="file" accept="image/*" disabled={importing||waiting} onChange={e=>{chooseImage(e.target.files?.[0]);e.target.value=''}}/></label><button className={voice.listening?'primary':'secondary'} disabled={waiting} onClick={voice.toggle}>{voice.listening?<Square size={17}/>:<Mic size={19}/>} {voice.listening?'Dừng ghi giọng nói':'Nhập bằng giọng nói'}</button></div>
 ...
 {mode==='local'&&<div className="notice">Chọn GPT, Groq hoặc Gemini trong Cài đặt → API key. Bạn có thể dùng key riêng mà chưa cần kết nối Firebase.</div>}</div>
